"""CRL with Twin Critics + Delayed Policy + Target Smoothing (FULL).

The complete TD3-for-CRL combination:
  - Twin (sa_encoder, g_encoder) pairs; each pair has its own EMA target
  ...
  - EMA update runs every critic step (independent of policy_delay).
...
from jaxgcrl.agents.crl_tc_ts.crl_tc_ts import update_actor_and_alpha_tc_ts
...
@dataclass
class CRL_FULL:
    """CRL with Twin Critics, Delayed Policy updates, and Target Smoothing.

    Combines all three TD3 adaptations applied to the contrastive critic:
    - Twin encoders + min-energy: resists overestimation of the contrastive value.
    - Delayed actor update: lets encoders converge before the policy follows them.
    - EMA target g_encoders: stabilises goal representations in the actor gradient.
    """
    ...
    def train_fn(
        ...
        def update_networks(carry, transitions):
            ...
            # Actor: min-energy with stop-grad EMA targets; gated by policy_delay
            def do_actor_update(ts):
                return update_actor_and_alpha_tc_ts(context, networks, transitions, ts, actor_key)
        ...
        training_walltime = 0
        logging.info("starting CRL_FULL training....")
