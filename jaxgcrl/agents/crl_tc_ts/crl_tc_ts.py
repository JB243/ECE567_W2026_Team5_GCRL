"""CRL with Twin Critics + Target Smoothing (TC+TS).

Combines:
  - Twin (sa_encoder, g_encoder) pairs; each pair gets its own EMA target
    g_encoder stored in target_g_encoder_params = {"target_g1": ..., "target_g2": ...}.
  - Actor uses min(energy(sa1, target_g1), energy(sa2, target_g2)) with
    stop_gradient on both target encoders.
  - Critic trains all four online encoders via summed InfoNCE losses.
  - EMA updates both target g_encoders every critic step.
"""
...
from jaxgcrl.agents.crl_tc.crl_tc import update_critic_tc
...
class TrainingState:
    ...
    target_g_encoder_params: Any   # {"target_g1": ..., "target_g2": ...}
...
# ---------------------------------------------------------------------------
# Actor loss: min-energy with stop-gradient EMA target g_encoders
# ---------------------------------------------------------------------------

def update_actor_and_alpha_tc_ts(config, networks, transitions, training_state, key):
    """Actor uses min(energy(sa1, target_g1), energy(sa2, target_g2))."""

    def actor_loss(actor_params, critic_params, target_g_params, log_alpha, transitions, key):
        ...
        # Pair 1: online sa_encoder, stop-grad target g_encoder
        sa_repr1 = networks["sa_encoder1"].apply(critic_params["sa_encoder1"], sa_input)
        g_repr1 = networks["g_encoder1"].apply(
            jax.lax.stop_gradient(target_g_params["target_g1"]), goal
        )
        q1 = _energy_fn(config["energy_fn"], sa_repr1, g_repr1)

        # Pair 2: online sa_encoder, stop-grad target g_encoder
        sa_repr2 = networks["sa_encoder2"].apply(critic_params["sa_encoder2"], sa_input)
        g_repr2 = networks["g_encoder2"].apply(
            jax.lax.stop_gradient(target_g_params["target_g2"]), goal
        )
...
@dataclass
class CRL_TC_TS:
    """CRL with Twin Critics and Target Smoothing.

    Each encoder pair has an EMA target g_encoder. The actor maximises
    min(energy with target_g1, energy with target_g2), combining
    overestimation resistance with stable goal representations.
    """
    ...
    def train_fn(
        ...
        sa_encoder1 = Encoder(**enc_cfg)
        g_encoder1  = Encoder(**enc_cfg)
        sa_encoder2 = Encoder(**enc_cfg)
        g_encoder2  = Encoder(**enc_cfg)

        g_params1_init = g_encoder1.init(g_key1, np.ones([1, goal_size]))
        g_params2_init = g_encoder2.init(g_key2, np.ones([1, goal_size]))

        critic_state = TrainState.create(
            apply_fn=None,
            params={
                "sa_encoder1": sa_encoder1.init(sa_key1, np.ones([1, state_size + action_size])),
                "g_encoder1":  g_params1_init,
                "sa_encoder2": sa_encoder2.init(sa_key2, np.ones([1, state_size + action_size])),
                "g_encoder2":  g_params2_init,
            },
        ...
        training_state = TrainingState(
            ...
            target_g_encoder_params={"target_g1": g_params1_init, "target_g2": g_params2_init},
        )
        ...
        def update_networks(carry, transitions):
            ...
            # Actor: min(energy1, energy2) with stop-grad EMA targets
            training_state, actor_metrics = update_actor_and_alpha_tc_ts(
                context, networks, transitions, training_state, actor_key
            )

            # Critic: trains all four online encoders
            training_state, critic_metrics = update_critic_tc(
                context, networks, transitions, training_state, critic_key
            )

            # EMA update for both target g_encoders
            new_target_g = {
                "target_g1": jax.tree_util.tree_map(
                    lambda t, o: (1.0 - tau) * t + tau * o,
                    training_state.target_g_encoder_params["target_g1"],
                    training_state.critic_state.params["g_encoder1"],
                ),
                "target_g2": jax.tree_util.tree_map(
                    lambda t, o: (1.0 - tau) * t + tau * o,
                    training_state.target_g_encoder_params["target_g2"],
                    training_state.critic_state.params["g_encoder2"],
                ),
            }
            training_state = training_state.replace(
        ...
        training_walltime = 0
        logging.info("starting CRL_TC_TS training....")
