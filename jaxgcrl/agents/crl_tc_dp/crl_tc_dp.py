"""CRL with Twin Critics + Delayed Policy updates (TC+DP).

Combines:
  - Twin (sa_encoder, g_encoder) pairs; actor uses min(energy1, energy2).
...
from jaxgcrl.agents.crl_tc.crl_tc import (
    update_critic_tc,
    update_actor_and_alpha_tc,
)
...
@dataclass
class CRL_TC_DP:
    """CRL with Twin Critics and Delayed Policy updates.

    Two encoder pairs resist energy overestimation; delayed actor updates
    let the encoders stabilise before the policy tracks them.
    """
    ...
    def train_fn(
        ...
        def update_networks(carry, transitions):
            ...
            def do_actor_update(ts):
                return update_actor_and_alpha_tc(context, networks, transitions, ts, actor_key)
        ...
        training_walltime = 0
        logging.info("starting CRL_TC_DP training....")
