"""CRL with Target Smoothing (TS) via a momentum goal encoder.

A Polyak-averaged (EMA) copy of g_encoder is maintained in TrainingState.
The critic trains the *online* g_encoder via InfoNCE as usual.
The actor gradient uses stop_grad(target_g_encoder) to evaluate goal
representations — this prevents the actor from chasing a rapidly-changing
goal embedding and stabilises the policy gradient signal.

The EMA update runs every critic step so the target tracks the online
encoder continuously. Everything else is identical to the base CRL.
"""
...
# ---------------------------------------------------------------------------
# Actor loss with stop-gradient target g_encoder
# ---------------------------------------------------------------------------

def update_actor_and_alpha_ts(config, networks, transitions, training_state, key):
    """Actor uses stop_grad(target_g_encoder) for stable goal representations."""
    ...
    def actor_loss(actor_params, critic_params, target_g_params, log_alpha, transitions, key):
        ...
        # Stop-gradient on the EMA target: actor gradient is stable across steps
        g_repr = networks["g_encoder"].apply(jax.lax.stop_gradient(target_g_params), goal)
        qf_pi = _energy_fn(config["energy_fn"], sa_repr, g_repr)
...
@dataclass
class CRL_TS:
    """CRL with Target Smoothing (momentum goal encoder).

    A Polyak-averaged EMA copy of g_encoder is maintained. The critic trains
    the online g_encoder normally. The actor policy gradient uses the
    stop-gradient EMA target to evaluate goal representations, preventing
    the actor from chasing rapidly-changing goal embeddings.
    """
    ...
    # Polyak averaging rate for the target g_encoder
    tau: float = 0.005
    ...
    def train_fn(
        ...
        g_encoder_params_init = g_encoder.init(g_key, np.ones([1, goal_size]))

        critic_state = TrainState.create(
            apply_fn=None,
            params={
                "sa_encoder": sa_encoder.init(sa_key, np.ones([1, state_size + action_size])),
                "g_encoder": g_encoder_params_init,
            },
        ...
        training_state = TrainingState(
            ...
            # Target starts identical to online; diverges via EMA
            target_g_encoder_params=g_encoder_params_init,
        )
        ...
        def update_networks(carry, transitions):
            ...
            # Actor uses target g_encoder (stop-grad EMA)
            training_state, actor_metrics = update_actor_and_alpha_ts(
                context, networks, transitions, training_state, actor_key
            )

            # Critic trains the online g_encoder normally
            training_state, critic_metrics = update_critic(
                context, networks, transitions, training_state, critic_key
            )

            # EMA update: target g_encoder tracks online g_encoder
        ...
        training_walltime = 0
        logging.info("starting CRL_TS training....")
