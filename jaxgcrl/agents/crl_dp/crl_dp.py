"""CRL with Delayed Policy updates (DP).

The critic (both encoders) is updated every gradient step. The actor and
entropy temperature are updated only every `policy_delay` critic steps via
jax.lax.cond, giving the encoders time to stabilise before the policy tracks
them. Everything else is identical to the base CRL.
"""
...
@dataclass
class CRL_DP:
    """CRL with Delayed Policy updates.

    The actor and entropy temperature (alpha) are updated only every
    `policy_delay` critic gradient steps. The critic runs every step.
    """
    ...
    # TD3-style delayed policy update
    policy_delay: int = 2
    ...
    def train_fn(
        ...
        def update_networks(carry, transitions):
            ...
            # Actor + alpha: updated every policy_delay critic steps
            batch_size = transitions.observation.shape[0]

            def do_actor_update(ts):
                return update_actor_and_alpha(context, networks, transitions, ts, actor_key)
        ...
        training_walltime = 0
        logging.info("starting CRL_DP training....")
